import { tools } from "../agent/tool"
import { executeTool } from "../agent/tool-executor"
import { prisma } from "../utils/db"
import { logger } from "../utils/logger"
import { loadPrompt } from "../utils/prompt-loader"
import { redis } from "../utils/redis"
import { sessionLogs } from "../utils/session-log"
import { createAgentCompletion } from "./openai.service"
...
export const processAgentChat = async (sessionId : string, question : string, correlationId : string, onChunk : (chunk : string) => void) => {
    ...
    //* Agent loop
    for(let i = 0; i < 5; i++){
        const msg = await createAgentCompletion(messages, tools)
