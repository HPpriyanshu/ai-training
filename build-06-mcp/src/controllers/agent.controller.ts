...
export const getConectedServersControllers = async (req : FastifyRequest, reply : FastifyReply) => {
    try {
        const data = await getConnectedServer()

        return reply.code(200).send(data)
    } catch (error) {
        req.log.error({error}, "Fetch server error")
        return reply.code(500).send({error})
    }
}